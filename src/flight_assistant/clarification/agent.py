...
**这一轮没被勾选的建议，不能就这么消失**——用户一次可能只想处理一两条，
不代表他不关心剩下的。除非用户明确说了"不用查这条了"，否则**下一轮的
items 里要把上一轮没选中的建议原样带回来**（可以跟这一轮新冒出来的
建议放在一起），让它们有机会被再次看到、再次选择，不要因为这次没勾选
就悄悄丢掉，变成用户永远不会再见到的建议。
